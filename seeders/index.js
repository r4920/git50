...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'tVM39ZdqiwpEK8L',
      'isDeleted':false,
      'username':'Ryley.Emmerich55',
      'email':'Henri_Tromp17@yahoo.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.updateOne(User, { 'username':'Ryley.Emmerich55' }, userToBeInserted,  { upsert: true });
    userToBeInserted = {
      'password':'sIMa9dQLywqBG0D',
      'isDeleted':false,
      'username':'Jeramy17',
      'email':'Cordie_White54@yahoo.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.updateOne(User, { 'username':'Jeramy17' }, userToBeInserted,  { upsert: true });
